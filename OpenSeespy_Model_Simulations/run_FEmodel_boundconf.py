"""OpenSees model for WSH6-based boundary-reinforcement configurations.

Fixed specimen properties are imported from :mod:`wsh6_reference`; boundary
reinforcement variables and LHS-sampled concrete strength/axial-load ratio are
passed to :func:`run_wall_model_BC`.
"""
...
def run_wall_model_BC(case_id, fc_MPa, ALR,
    ...
    # -------------------------------------------------------------------------
    # WSH6 REFERENCE GEOMETRY AND FIXED WEB REINFORCEMENT
    # -------------------------------------------------------------------------
    ...
    dbar1 = float(dbar1) * 1e-3
    no_bars_bl = int(no_bars_bl)

    # Web reinforcement remains fixed at the WSH6 reference layout.
    fy1 = ref.fy_boundary
    fy2 = ref.fy_web
    dhoop = ref.hoop_diameter
    dbar2 = ref.web_bar_diameter
    webspacing = ref.web_bar_spacing
    ...
    return {
        ...
        # Boundary reinforcement inputs
        "rho_bl": rho_bl,
        "rho_sh": rho_sh,
        "no_bars_bl": no_bars_bl,
        "dbar1_mm": float(dbar1) * 1000,
    
        # Web reinforcement kept from base specimen
        "rho_l": rho_l,
        "rho_t": rho_t,
        "dbar2_mm": float(dbar2) * 1000,
